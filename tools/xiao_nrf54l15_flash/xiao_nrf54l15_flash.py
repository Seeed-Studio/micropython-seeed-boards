...
@details This script provides core functionality to erase and flash nRF54L15 devices.
                 Features:
                     * Auto upgrade/install dependencies (intelhex, pyocd) unless SKIP_PYOCD_UPGRADE=1.
                     * Automatic HEX discovery if --hex not specified:
                             1. Use ./merged.hex if存在
                             2. 若仅发现一个 *.hex 则使用
                             3. 若多个，选择最近修改时间的
                     * Always performs mass erase BEFORE programming.
                     * Single-command usage: python xiao_nrf54l15_flash.py
@note Set environment variable SKIP_PYOCD_UPGRADE=1 to disable auto-upgrade.
@example python xiao_nrf54l15_flash.py            # auto-detect HEX and mass erase + program
@example python xiao_nrf54l15_flash.py --hex app.hex --probe <probe_id>
"""
...
def ensure_latest(packages: List[str]):
    """Ensure packages are installed & upgraded to latest unless SKIP_PYOCD_UPGRADE=1 is set.

    We intentionally run an upgrade each invocation for simplicity, but allow opt-out.
    """
    ...
    for pkg in packages:
        try:
            # If import succeeds we still attempt upgrade to ensure latest.
            import_module(pkg)
        except Exception:
            pass
        ...
        try:
            ...
            print(f"[WARN] Failed to upgrade {pkg}: {e}. Continuing with currently installed version if available.")
            try:
                import_module(pkg)
            except Exception:
...
# Always attempt to ensure dependencies before importing them.
...
def main():
    ...
    parser.add_argument(
        "--hex",
        help="Path to the HEX file to be programmed. If omitted, auto-detect in current directory.",
    )
    parser.add_argument(
        "--probe", 
    ...
    try:
        # Auto-detect hex if not provided
        if not args.hex:
            cwd = os.getcwd()
            candidate = None
            merged_path = os.path.join(cwd, "merged.hex")
            if os.path.isfile(merged_path):
                candidate = merged_path
                logger.info(f"Auto-selected HEX: {candidate} (found merged.hex)")
            else:
                hex_files = [f for f in os.listdir(cwd) if f.lower().endswith('.hex')]
                if len(hex_files) == 0:
                    logger.error("No HEX file found in current directory. Provide --hex or add a .hex file.")
                    sys.exit(1)
                elif len(hex_files) == 1:
                    candidate = os.path.join(cwd, hex_files[0])
                    logger.info(f"Auto-selected HEX: {candidate} (only hex file)")
                else:
                    # pick most recently modified
                    hex_files_full = [os.path.join(cwd, f) for f in hex_files]
                    hex_files_full.sort(key=lambda p: os.path.getmtime(p), reverse=True)
                    candidate = hex_files_full[0]
                    logger.info(f"Auto-selected HEX: {candidate} (most recently modified)")
            args.hex = candidate

        logger.info(f"Using HEX file: {args.hex}")
